...
$(".btn_retirar").click(function(){
    ...
    var serial = $(this).closest('tr').find(".serie2"); 
...
$("#btn_cerrar_r").click(function () {
    var tipo_movimiento = $("#tip_mov").val();

    if (validate_retiros()) {
        consumir_servicio_pc_track(); //servicio de cambio de equipo

        var listInfo = getSeriesDetalle();
        if (listInfo.length > 0 || listInfo == null || listInfo.length == 0) {
...
$("#btn-confirmacion").click(function(){
    ...
    $("#btn_cerrar_r").attr('disabled', 'disabled');
    
    inactive_customer(listInfo);//llamado a funcion

    ajax_cambios(id_doc, listInfo);//se guardan los productos con cambios
});
    
// public JsonResult Change_Client_Status_Client(string id_client = null, string type="")
...
//inactivar afiliado por retirar todos los productos
function inactive_customer(listInfo) {
    var cantproducts = $("#cant_products").val();
    if (cantproducts > 0 && cantproducts >= listInfo.length) {
        ajax_inactivate_client();
    }
}
    
function ajax_inactivate_client() {
    $.ajax({
        ...
        data:
            {
                id_client: $("#membership_number").val(),
                estatus: "I"
            },
        success: function (result) {
        },
        error: function (XMLHttpRequest) {
            alert("Error al  inactivar el afiliado");
            successful = false;
        }
    });
}
...
function ajax_cambios(num_doc, products_array) {
    ...
    if(successful == true){
        //llamado al ajax de pc track para cierre*************************************************
        consumir_servicio_pc_track_retiro(listInfo);
        $("#formR").submit();
    }
    return false;
}
...
$("#btn_cambiar_serie").click(function () {
    ...
    if (nueva_serie.length>0) {
        ...
        cambiar_serie(old_serie, nueva_serie, num_ticket, consecutive, category);
    }
    else {
...
function validar_series() {/*validar paste*/
    ...
    $('input[name=serie2]').each(function () {
        ...
        if(serial != "---SIN SERIE---" && !$(this).attr('disabled')){
            if (serial != inserted) {
...
function validate_retiros() {
    var retirar = 0;
    $('.btn_retirar').each(function(){
...
//servicio de retiro de series
function consumir_servicio_pc_track_retiro(lista) {
    if (lista.length > 0) {
        var afiliado = $("#membership_number").val();
        $.ajax({
            url: "/Product/Call_External_Service",
